...
class InfiniBertSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        if config.hidden_size % config.num_attention_heads != 0 and not hasattr(config, "embedding_size"):
            raise ValueError(
...
class InfiniBertAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = InfiniBertSelfAttention(config, position_embedding_type=position_embedding_type)
    ...
    def forward(
        ...
        self_outputs, new_memories = self.self(
            hidden_states,
            attention_mask,
            head_mask,
...
class InfiniBertLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = InfiniBertAttention(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = InfiniBertAttention(config, position_embedding_type="absolute")
...
class InfiniBertEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([InfiniBertLayer(config) for _ in range(config.num_hidden_layers)])
...
class InfiniBertModel(BertPreTrainedModel):
    """
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.encoder = InfiniBertEncoder(config)
