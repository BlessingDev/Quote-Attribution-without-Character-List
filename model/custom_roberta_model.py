...
## Infini-Attention 적용.
class InfiniRobertaSelfAttention(nn.Module):
    ...
    def forward(
        ...
        ##
        # query_layer, key_layer, value_layer 차원 (b h n d)
        # 출력 차원 (b h n d)
        context_layer = self.fastweight_mem.retrieve_and_add_to_output(context_layer, query_layer, past_memories)
        ##
        ...
        ##
        new_memories = self.fastweight_mem.create_new_memories(key_layer, value_layer, past_memories)
        ##
...
class InfiniRobertaAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = InfiniRobertaSelfAttention(config, position_embedding_type=position_embedding_type)
    ...
    def forward(
        ...
        self_outputs, new_mem = self.self(
            hidden_states,
            attention_mask,
            head_mask,
...
class InfiniRobertaLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = InfiniRobertaAttention(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = InfiniRobertaAttention(config, position_embedding_type="absolute")
        self.intermediate = RobertaIntermediate(config)
        self.output = RobertaOutput(config)
        self.past_memories = None
    ...
    def forward(
        ...
        self_attention_outputs, self.past_memories = self.attention(
            hidden_states,
            attention_mask,
            head_mask,
            output_attentions=output_attentions,
            past_key_value=self_attn_past_key_value,
            past_memories=self.past_memories,
        )
    ...
    def detatch_memories(self):
        self.past_memories.kv_mem.detach_()
        self.past_memories.k_norm.detach_()


# Copied from transformers.models.bert.modeling_bert.BertEncoder with Bert->Roberta
class InfiniRobertaEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([InfiniRobertaLayer(config) for _ in range(config.num_hidden_layers)])
    ...
    def init_memories(self):
        for i, layer_module in enumerate(self.layer):
            layer_module.init_memories()
    
    def detach_memories_(self):
        for i, layer_module in enumerate(self.layer):
            layer_module.detatch_memories()
...
class InfiniRobertaPreTrainedModel(PreTrainedModel):
    """
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, InfiniRobertaEncoder):
            module.gradient_checkpointing = value

class InfiniRobertaModel(InfiniRobertaPreTrainedModel):
    """
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.encoder = InfiniRobertaEncoder(config)
